...
# Connect to Razorpay using keys from settings.py
...
def payment_page(request, show_id):
    ...
    # Razorpay needs amount in PAISE (₹1 = 100 paise, so ₹250 = 25000)
    amount_in_paise = int(ticket_total * 100)
    try:
    ...
    context = {
        ...
        # NEW — Razorpay data for the HTML template
...
def razorpay_callback(request):
    ...
    food_orders_json    = request.POST.get('food_orders', '[]')
    coupon_code         = request.POST.get('coupon_code', '')

    # VERIFY payment is genuine (security check)
    try:
        ...
        return redirect('bookings:payment', show_id=show_id)

    # Payment is real — now create booking
    show = get_object_or_404(Show, id=show_id)
    held_seats = SeatBooking.objects.filter(show=show, held_by=request.user, status='hold')

    if not held_seats.exists():
        messages.error(request, 'Session expired. Please re-select seats.')
        return redirect('bookings:seat_selection', show_id=show_id)
    ...
    try:
        for fo in json.loads(food_orders_json):
            try:
    ...
    # Generate QR code
    qr_data = f"BOOKING:{booking.booking_id}|USER:{request.user.username}|PAY:{razorpay_payment_id}"
    qr_img = qrcode.make(qr_data)
    buffer = BytesIO()
    qr_img.save(buffer, format='PNG')
    booking.qr_code.save(f'qr_{booking.booking_id}.png', ContentFile(buffer.getvalue()), save=True)

    # Notification
    Notification.objects.create(
        user=request.user,
        title='Booking Confirmed!',
        message=f'Your booking for {show.movie.title} is confirmed. Seats: {", ".join(seat_labels)}',
        type='booking',
        booking=booking,
    )

    messages.success(request, 'Payment successful! Your ticket is confirmed.')
    return redirect('bookings:ticket_confirmation', booking_id=booking.booking_id)
